...
class LQR_Env(gym.Env):
    ...
    def interference_packet_delivery_rate(self, H, actions, traffic_loads=None):
        ...
        # 根据图片中的公式计算干扰带来的信道容量损失，并转换为延迟损失
        # Capacity Loss = B log2(1 + S/N) - B log2(1 + S/(N+I))
        # Delay Loss = traffic_load/channel_rate - traffic_load/(channel_rate + rate_loss_suffered)
        ...
        if traffic_loads is None:
            traffic_loads = np.full(self.num_users, 3000 * 8)  # 默认流量负载
        ...
        # 返回SINR、固定PDR值（1.0表示100%传输成功率）、信道速率和延迟损失
        # 去除PDR约束，所有链路都假设100%传输成功
        pdr = np.ones(self.num_users)  # 固定为1.0，表示无PDR约束
    ...
    def get_observation_space_info(self):
        ...
        info = {
            ...
            'components': {
                'channel_matrix': self.channel_state_dim,  # num_users^2
    ...
    def _update_control_states_downlink(self, downlink_power, H, lambd, action_penalty):
        ...
        # 基于信道速率计算奖励
        # 奖励函数：最大化系统总信道速率，减少干扰带来的延迟损失
        system_total_rate = float(np.sum(channel_rates))
        system_avg_rate = float(np.mean(channel_rates))
        
        # 计算干扰带来的延迟损失
        total_delay_loss_suffered = float(np.sum(delay_loss_suffered))

        # 奖励函数：系统信道速率 - 干扰延迟损失惩罚 - 约束惩罚
        w_rate = 1.0           # 系统速率权重
        w_loss = 1.0

        # 计算延迟奖励（平均链路延迟）
        delay_reward = np.mean(self.link_delays)
        # 计算延迟损失（干扰带来的延迟损失）
        delay_loss = total_delay_loss_suffered
        # 最终奖励：-(延迟奖励 + 延迟损失)
        reward = -(delay_reward + delay_loss)
        
        reward /= 10  # 缩放奖励值，稳定训练
        # 保存系统级指标
        self.system_avg_rate = system_avg_rate
        self.total_delay_loss_suffered = total_delay_loss_suffered
