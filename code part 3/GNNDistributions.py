...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution_net(self, latent_dim: int, log_std_init: float = 0.0) -> Tuple[nn.Module, nn.Parameter]:
        ...
        :return:
        """
        mean_actions = nn.Linear(latent_dim, self.action_dim)                       # (100, )
        log_std = th.ones(self.action_dim)*log_std_init
        #nn.Parameter(th.ones(self.action_dim) * log_std_init, requires_grad=True)  # (200, )
        return mean_actions, log_std

    def make_log_std(self, log_std_init: float = 0.0) -> nn.Parameter:
        """
        :param log_std_init: Initial value for the log standard deviation
        :return:
        """
        log_std = th.ones(self.action_dim)*log_std_init #nn.Parameter(th.ones(self.action_dim) * log_std_init, requires_grad=True)
        return log_std
    ...
    def proba_distribution(self, mean_actions: th.Tensor, log_std: th.Tensor) -> "DiagGaussianDistribution":
        ...
        """
        # print(mean_actions)
        # print(th.ones_like(mean_actions))
        # print(log_std.exp())
...
def make_proba_distribution(
    action_space: gym.spaces.Space, n_users: int = 10, n_features_action: int = 1
) -> Distribution:
    """
    ...
    if isinstance(action_space, spaces.Box):
        # Support both 1D and 2D action spaces
        if len(action_space.shape) == 1:
            # For 1D action space, treat it as (num_users, 1)
            cls = DiagGaussianDistribution
            return cls(get_action_dim(action_space))
        elif len(action_space.shape) == 2:
            # For 2D action space, use as is
            cls = DiagGaussianDistribution
            return cls(get_action_dim(action_space))
        else:
            raise ValueError(f"Action space must be 1D or 2D, got shape {action_space.shape}")
