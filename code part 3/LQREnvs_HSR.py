...
class LQR_Env(gym.Env):
    def __init__(self, num_users, upperbound, constraint_dim, L, assign, mu=1, T=40,
                 ...
                 # 卫星相关参数
                 satellite_enabled=True, leo1_altitude=800e3, leo2_altitude=1200e3,
                 satellite_freq_hz=30e9, satellite1_bandwidth=400e6, satellite2_bandwidth=400e6,
                 c_light=3e8, k_boltzmann=1.38e-23,
                 satellite_noise_figure_db=1.2, satellite_tx_power_init=2.0,
        ...
        if self.satellite_enabled:
            ...
            self.satellite_noise_temperature = T0 * (10 ** (self.satellite_noise_figure / 10) - 1)
        ...
        # 卫星相关状态变量
        self.link_satellite_status = np.zeros(self.num_users, dtype=int)  # 卫星连接状态
        self.user_traffic_types = np.random.choice([0, 1], self.num_users, p=[0.7, 0.3])  # 用户业务类型：0=Others, 1=HVFT
        self.hvft_accumulated_data = np.zeros(self.num_users)  # HVFT累积数据
        self.hvft_accumulation_rounds = np.zeros(self.num_users)  # HVFT累积轮数
        self.hvft_transmission_decisions = np.zeros(self.num_users, dtype=int)  # HVFT传输决策
        self.hvft_satellite_transmission = np.zeros(self.num_users, dtype=int)  # HVFT卫星传输
    ...
    def interference_SINR(self, H_matrix, actions):
        ...
        for i in range(num_users):
            ...
            if i == 0:
                print(f"device_idx: {i}, serving_bs_idx: {serving_bs_idx}, W_ICI_link: {W_ICI_link}")
            # 3. 计算设备i受到的干扰功率
    ...
    def calculate_satellite_throughput(self, satellite_bandwidth_system):
        """
        计算卫星下行链路吞吐量 - 只考虑下行链路
        
        在高速铁路场景中，主要流量是下行（下载），上行流量相对较少，
        因此可以简化为只考虑下行链路的性能。
        """
        ...
        # 只考虑下行链路：卫星 -> 终端设备
        # 路径损耗计算
        path_loss_db = 20 * np.log10(4 * np.pi * self.leo1_altitude / (self.c_light / self.satellite_freq_hz))
        
        # 下行链路SNR计算
        # 卫星发射功率 + 卫星天线增益 - 路径损耗 + 终端G/T - 终端噪声系数
        SNR_db = (self.satellite_tx_power + 
                          10*np.log10(self.satellite_antenna_gain_tx) - 
                          path_loss_db + 
                          self.satellite_g_over_t - 
                          self.satellite_noise_figure)
        
        # 转换为线性值
        SNR_linear = 10 ** (SNR_db / 10)
        
        # 计算下行链路容量
        throughput = satellite_bandwidth_system * np.log2(1 + SNR_linear)
        
        return throughput

    def intelligent_satellite_selection(self, channel_rates, ground_delays, traffic_loads, user_traffic_types):
        """
        智能卫星选择策略：让太差的链路连接至卫星
        ...
        # 计算每个用户的"链路质量分数"
        link_quality_scores = np.zeros(self.num_users)
        
        for i in range(self.num_users):
            # 1. 信道质量分数（归一化）
            channel_quality = channel_rates[i] / (self.bandwidth * np.log2(1 + 10))  # 归一化到[0,1]
            
            # 2. 延迟分数（延迟越高，分数越低）
            delay_score = 1.0 - min(ground_delays[i] / 3.0, 1.0)  # 归一化到[0,1]
            
            # 3. 流量负载分数（负载越重，分数越高）
            load_score = min(traffic_loads[i] / (3000 * 8), 1.0)  # 归一化到[0,1]
            
            # 4. 业务类型权重
            traffic_weight = 2.0 if user_traffic_types[i] == 1 else 1.0  # HVFT权重更高
            
            # 综合链路质量分数（越低表示链路越差）
            link_quality_scores[i] = (channel_quality * 0.4 + delay_score * 0.4 + load_score * 0.2) * traffic_weight
        
        # 选择链路质量最差的用户使用卫星
        # 卫星容量限制：最多选择satellite_subchannel_num个用户
        worst_indices = np.argsort(link_quality_scores)[:min(self.satellite_subchannel_num, self.num_users)]
    ...
    def calculate_enhanced_satellite_delays(self, satellite_mask, ground_delays, traffic_loads):
        """
        增强的卫星延迟计算，包含0.08-0.12秒随机波动
        """
        final_delays = np.copy(ground_delays)
        satellite_status = np.zeros(self.num_users, dtype=int)
        
        if not self.satellite_enabled or not np.any(satellite_mask):
            return final_delays, satellite_status
        ...
        for user_idx in satellite_users:
            propagation_delay = base_trip_delay
            fluctuation_delay = np.random.uniform(0.08, 0.12)
            satellite_capacity = self.calculate_satellite_throughput(self.satellite1_bandwidth)
            
            if satellite_capacity > 0:
                transmission_delay = traffic_loads[user_idx] / satellite_capacity
            else:
                transmission_delay = 3.0  # 最大延迟

            total_satellite_delay = propagation_delay + fluctuation_delay + transmission_delay
            
            # 5. 与地面延迟比较，选择更优的
            if total_satellite_delay < ground_delays[user_idx]:
                final_delays[user_idx] = total_satellite_delay
                satellite_status[user_idx] = 1
            else:
                # 卫星延迟更高，保持地面连接
                satellite_status[user_idx] = 0
        
        return final_delays, satellite_status

    def calculate_current_channel_rates(self, power_allocation):
        """计算当前功率分配下的信道速率"""
        SINR, channel_rates, _ = self.interference_SINR(self.H, power_allocation)
        return channel_rates

    def calculate_ground_delays(self, channel_rates):
        """计算地面链路延迟"""
        transmission_delays = self.link_delay(channel_rates, self.traffic_loads)
        rtt_delays = self.hsr_rtt_delay(self.num_users)
        return transmission_delays + rtt_delays

    def calculate_satellite_priorities(self, channel_rates, ground_delays):
        """计算卫星优先级分数"""
        priorities = np.zeros(self.num_users)
        for i in range(self.num_users):
            # 信道质量分数（越低越好）
            channel_quality = channel_rates[i] / (self.bandwidth * np.log2(1 + 10))
            # 延迟分数（越高越好）
            delay_score = ground_delays[i] / 3.0
            # 业务类型权重
            traffic_weight = 2.0 if self.user_traffic_types[i] == 1 else 1.0
            # 综合优先级（越高表示越需要卫星）
            priorities[i] = (delay_score - channel_quality) * traffic_weight
        return priorities

    def enforce_satellite_capacity_constraint(self, agent_satellite_mask):
        """
        执行卫星容量约束：确保不超过最大连接数
        
        环境只负责执行约束，不干预智能体的决策逻辑。
        如果智能体选择的用户数超过限制，按用户索引顺序保留前N个用户。
        
        Args:
            agent_satellite_mask: 智能体的卫星选择决策 (bool array)
            
        Returns:
            final_satellite_mask: 执行约束后的最终卫星选择 (bool array)
        """
        if not self.satellite_enabled:
            return np.zeros(self.num_users, dtype=bool)
        
        # 如果智能体选择的卫星用户数不超过限制，直接返回
        if np.sum(agent_satellite_mask) <= self.satellite_subchannel_num:
            return agent_satellite_mask
        
        # 如果超过限制，按用户索引顺序选择前N个用户
        # 这样智能体可以学习到：选择用户时需要考虑索引顺序
        selected_users = np.where(agent_satellite_mask)[0]
        final_users = selected_users[:self.satellite_subchannel_num]
        
        # 构建最终的卫星掩码
        final_satellite_mask = np.zeros(self.num_users, dtype=bool)
        final_satellite_mask[final_users] = True
        
        return final_satellite_mask
    ...
    def _update_control_states_downlink(self, downlink_power, H, lambd, action_penalty, satellite_mask=None):
        self.time_step += 1
        SINR,channel_rates,interference_plus_noise = self.interference_SINR(H, downlink_power)
        
        # 计算地面链路延迟
        ground_transmission_delays = self.link_delay(channel_rates, self.traffic_loads)
        ground_rtt_delays = self.hsr_rtt_delay(self.num_users)
        ground_total_delays = ground_transmission_delays + ground_rtt_delays
        
        # 智能卫星选择（如果没有提供卫星掩码）
        if satellite_mask is None:
            satellite_mask = self.intelligent_satellite_selection(
                channel_rates, ground_total_delays, self.traffic_loads, self.user_traffic_types
            )
        
        # 计算最终延迟（考虑卫星）
        final_delays, satellite_status = self.calculate_enhanced_satellite_delays(
            satellite_mask, ground_total_delays, self.traffic_loads
        )
        
        # 更新状态变量
        self.link_delays = final_delays
        self.rtt_delays = ground_rtt_delays
...
class LQRAdHocDownlink(LQR_Env):
    def __init__(self, num_users, upperbound, constraint_dim, L, assign, n, k, mu=1, T=40,
                 ...
                 snr_threshold_eta=10.0, max_neighbors=5, satellite_enabled=True, **satellite_params):
        ...
        super().__init__(num_users, upperbound, constraint_dim, L, assign, mu=mu, T=T,
                         ...
                         satellite_enabled=satellite_enabled, **satellite_params)

        # 简化的动作空间：(num_users, 2)
        # [:, 0]: 地面功率分配 [-1, 1]
        # [:, 1]: 卫星选择决策 [-1, 1] -> {0, 1}
        self.action_space = spaces.Box(
            low=-np.ones((num_users, 2)), 
            high=np.ones((num_users, 2))
        )
        self.n = n
        self.k = k
    ...
    def step(self, action):
        ...
        # 解析动作
        power_raw = action[:, 0]      # 功率分配决策
        satellite_raw = action[:, 1]  # 卫星选择决策
        
        # 处理功率分配
        power_action = self.scale_power(power_raw, self.force_max_power)
        self.agent_satellite_mask = np.round(np.clip(satellite_raw, 0, 1)).astype(bool)
        # 环境只负责执行容量约束：确保不超过最大连接数
        self.final_satellite_mask = self.enforce_satellite_capacity_constraint(self.agent_satellite_mask)
        ...
        SINR, one_step_reward, done = \
            self._update_control_states_downlink(power_action, self.H, lambd, action_penalty, self.final_satellite_mask)
        ...
        info = {
            ...
            # 卫星相关
            'satellite_users_count': int(np.sum(self.link_satellite_status)),
            'ground_users_count': int(self.num_users - np.sum(self.link_satellite_status)),
            'satellite_utilization': float(np.sum(self.link_satellite_status) / self.satellite_subchannel_num),
            'satellite_capacity_violation': int(np.sum(self.agent_satellite_mask) - np.sum(self.final_satellite_mask)),
