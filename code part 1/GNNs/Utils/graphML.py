...
import GNNs.Utils.graphTools as graphTools
...
def learnAttentionGSO(x, a, W, S, negative_slope=0.2):
    ...
    maskEdges = maskEdges.to(x.device)
...
def LSIGF_B(h, S, x, b=None):
    """
    LSIGF_B(filter_taps, GSO, input, bias=None) Computes the output of a
        linear shift-invariant graph filter (graph convolution) on time-varying GSO
        and then adds bias.

    Denote as G the number of input features, F the number of output features,
    E the number of edge features, K the number of filter taps, N the number of
    nodes, S_{e} in R^{N x N} the GSO for edge feature e,
    x(t) in R^{G x N} the input data at time t where x_{g}(t) in R^{N} is the
    graph signal representing feature g, and b in R^{F x N} the bias vector,
    with b_{f} in R^{N} representing the bias for feature f.
    ...
    Outputs:
        output: filtered signals; shape:
            batch_size x time_samples x output_features x number_nodes
    """
    # This is the LSIGF with (B)atch capabilities (i.e. there is
    # a different GSO for each element in the batch. The GSO should be
    # transparent).
    ...
    # B x E x K x G x N.
    # Let's start by reshaping x so it can be multiplied by S
    x = x.reshape([B, 1, G, N]).repeat(1, E, 1, 1)
    # x.reshape([B, 1, 1, G, N]).repeat(1, E, 1, 1, 1)

    # Now, for the first value of k, we just have the same signal
    z = x.reshape([B, 1, E, G, N])

    # Now we need to start multiplying with S
    for k in range(1, K):
        x = torch.matmul(x, S) # B x E x G x N
        # Add the dimension along K
        xS = x.reshape(B, 1, E, G, N) # B x 1 x E x G x N
        # And concatenate it with z
        z = torch.cat((z, xS), dim=1)

    # Now, we finally made it to a vector z of shape B x K x E x G x N
    # To finally multiply with the filter taps, we need to swap the sizes
    # and reshape
    z = z.permute(0, 4, 2, 1, 3)  # B x N x E x K x G
    ...
    # Multiply
    y = torch.matmul(z, h)  # B x N x F
    # And permute
    y = y.permute(0, 2, 1)  # B x F x N
    # Finally, add the bias
    if b is not None:
        y = y + b
...
class GraphFilter_B(nn.Module):
    # Vinicius Lima, vlima@seas.upenn.edu
    """
    GraphFilter_B Creates a (linear) layer that applies a graph filter (i.e. a
        graph convolution) considering batches of (possibly time-varying) GSO.

    Initialization:

        GraphFilter_B(in_features, out_features, filter_taps,
                       edge_features=1, bias=True)
    ...
    Add graph shift operator:
        ...
        GraphFilter_B.addGSO(GSO) Before applying the filter, we need to define
        the GSO that we are going to use. This allows to change the GSO while
        using the same filtering coefficients (as long as the number of edge
        features is the same; but the number of nodes can change).
    ...
    def forward(self, x):
        ...
        assert x.shape[2] == self.N
        # Compute the filter output
        u = LSIGF_B(self.weight, self.S, x, self.bias)
        # u is of shape batchSize x dimOutFeatures x numberNodes
        return u
