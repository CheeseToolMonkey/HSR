...
import GNNs.Utils.graphTools

from GNNs.Utils.dataTools import changeDataType

zeroTolerance = 1e-9  # Absolute values below this number are considered zero.

class DGNN(nn.Module):
    # Dynamic Graph Neural Network
    def __init__(self, dimNodeSignals, nFilterTaps, bias, nonlinearity,
                     nSelectedNodes, poolingFunction, poolingSize, dimLayersMLP, GSO, order=None):
            super(DGNN, self).__init__()

            # Initialize parameters
            self.dimNodeSignals = dimNodeSignals
            ...
            assert len(dimNodeSignals) == len(nFilterTaps) + 1, "dimNodeSignals and nFilterTaps length mismatch"
            assert len(nSelectedNodes) == len(poolingSize), "nSelectedNodes and poolingSize length mismatch"

            # Initialize graph convolutional layers
            self.graphConvLayers = nn.ModuleList()
            for i in range(len(nFilterTaps)):
                self.graphConvLayers.append(nn.Conv1d(dimNodeSignals[i], dimNodeSignals[i+1], kernel_size=nFilterTaps[i], bias=bias))

            # Initialize pooling layers
            self.poolingLayers = nn.ModuleList()
            for i in range(len(poolingSize)):
                self.poolingLayers.append(poolingFunction(kernel_size=poolingSize[i]))

            # Initialize MLP layers
            self.MLP = nn.Sequential()
            input_dim = dimNodeSignals[-1] * nSelectedNodes[-1]
            for i in range(len(dimLayersMLP)):
                self.MLP.add_module('fc{}'.format(i), nn.Linear(input_dim if i == 0 else dimLayersMLP[i-1], dimLayersMLP[i]))
                if i < len(dimLayersMLP) - 1:
                    self.MLP.add_module('relu{}'.format(i), nonlinearity())

    def forward(self, x):
        # Graph convolution and pooling layers
        for i in range(len(self.graphConvLayers)):
            x = self.graphConvLayers[i](x)
    ...
    def add_edge(self, edge, weight):
        i, j = edge
        self.GSO[0, i, j] = weight
        if not self.is_directed:  # 如果是无向图
            self.GSO[0, j, i] = weight
    ...
    def update_node_features(self, node_index, new_features):
        if node_index < len(self.data['node_features']) - 1:  # 基站更新
            self.data['node_features'][node_index] = new_features
        else:  # 设备更新
            self.data['node_features'][-1] = new_features
        # 重新计算路径损失
        for i in range(len(self.data['node_features']) - 1):  # 遍历所有基站
            d = np.linalg.norm(self.data['node_features'][i] - self.data['node_features'][-1])  # 基站到设备的距离
            self.data['edge_features'][i, 0] = np.power(d, -self.pl)
...
class SelectionGNN(nn.Module):
    ...
    def __init__(self,
        ...
        if order is not None:
            ...
            self.permFunction = GNNs.Utils.graphTools.permIdentity
        ...
        if self.coarsening and self.E == 1:
            self.permFunction = GNNs.Utils.graphTools.permCoarsening  # Override
            # permutation function for the one corresponding to coarsening
            GSO = scipy.sparse.csr_matrix(GSO[0])
            GSO, self.order = GNNs.Utils.graphTools.coarsen(GSO, levels=self.L,
    ...
    def splitForward(self, x):
        ...
        if x.shape[2] != self.N[0] and self.coarsening:
            ...
            x = GNNs.Utils.graphTools.permCoarsening(x, self.order)
...
class GatedGraphRecurrentNNHiddenState(nn.Module):
    # og by Luana R. Ruiz, rubruiz@seas.upenn.edu, 2021/03/04
    # customizing to define hidden state outside, Vinicius Lima, vlima@seas.upenn.edu
    """
   GatedGraphRecurrentNN: implements the (time, node, edge)-gated GRNN
   architecture. It is a single-layer gated GRNN and the hidden state is
   initialized at random drawing from a standard gaussian.
    ...
    def __init__(self,
        ...
        self.S = GSO
        if 'torch' not in repr(self.S.dtype):
            self.S = torch.tensor(self.S).float()
    ...
    def splitForward(self, x, z_t):
        ...
        y = self.Readout(y)  # B x T x N x dimReadout[-1]
        # Reshape and return
        return y.permute(0, 1, 3, 2), yOut, z
        # B x T x dimReadout[-1] x N, B x T x dimFeatures[-1] x N
    ...
    def forward(self, x, z_t):
        ...
        # (the GNN and the MLP) and returns only the MLP output in the proper
        # forward function.
        output, _, z_t1 = self.splitForward(x, z_t)
...
class GatedGraphRecurrentNNHiddenStateSeq(nn.Module):
    # og by Luana R. Ruiz, rubruiz@seas.upenn.edu, 2021/03/04
    # customizing to define hidden state outside, Vinicius Lima, vlima@seas.upenn.edu
    # temporary solution to avoid comparisons / reshapes when keeping track of time/sequence hidden state
    """
   GatedGraphRecurrentNN: implements the (time, node, edge)-gated GRNN
   architecture. It is a single-layer gated GRNN and the hidden state is
   initialized at random drawing from a standard gaussian.
    ...
    def splitForward(self, x, z_t):
        ...
        y = self.Readout(y)  # B x T x N x dimReadout[-1]
        # Reshape and return
        return y.permute(0, 1, 3, 2), yOut, z_h
        # B x T x dimReadout[-1] x N, B x T x dimFeatures[-1] x N
